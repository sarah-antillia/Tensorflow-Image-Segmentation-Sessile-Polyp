...
class GrayScaleImageWriter:
  ...
  def save(self, data, output_dir, basename, factor=255.0):
    image = self.mask_to_image(data, factor=factor)
    image_filepath = os.path.join(output_dir, basename)
    image.save(image_filepath)
    print("=== Saved {}". format(image_filepath))


  def save_resized(self, data, resized, output_dir, basename, factor=255.0):
    image = self.mask_to_image(data, factor=factor)
    image_filepath = os.path.join(output_dir, basename )
